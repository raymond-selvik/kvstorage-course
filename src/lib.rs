...
impl KvStore {
    ...
    pub fn remove(&mut self, key: String) -> Result<()> {
        ...
        self.write_entry(Command::RM, key, String::from("removed"))
            .unwrap();
        Ok(())
    }
    ...
    fn get_reader(
        ...
        serde_json::StreamDeserializer<'static, serde_json::de::IoRead<BufReader<File>>, Entry>,
    > {
        let file = File::open(self.file_path.as_path()).unwrap();
        let reader = BufReader::new(file);
        let mut iterator = serde_json::Deserializer::from_reader(reader)
            .into_iter::<Entry>()
            .enumerate();
        iterator
    }
